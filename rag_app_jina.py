...
EMBEDDING_MODEL_NAME = "jinaai/jina-embeddings-v4" # all-mpnet-base-v2 # jinaai/jina-embeddings-v4
...
class LocalEmbedder:
    def __init__(self, model_name: str = EMBEDDING_MODEL_NAME):
        ...
        # если это jina-embeddings-v4, используем спец. режим
        self.is_jina = "jina" in model_name.lower()

    def embed_texts(self, texts: List[str], is_query: bool = False) -> np.ndarray:
        """
        Возвращает numpy-массив эмбеддингов (float32).
        Для Jina embeddings — указываем task="retrieval" и prompt_name.
        """
        if self.is_jina:
            task = "retrieval"
...
def retrieve_relevant_chunks(question: str,
    ...
    """
    q_emb = embedder.embed_text(question, is_query=True).astype("float32")
    # normalize
